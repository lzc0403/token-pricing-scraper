...
- **2026-08-18 git 损坏彻底修复 + 全量推送**：08-16 的修复不彻底，`git gc`/`git fetch` 仍报 `Could not read 66d113295082439e0050ba107588f5af153191e9` + `unresolved deltas`。本次走完整 reclone 替换：备份工作区→`git clone --depth 5 origin C:/Users/昊哥/AppData/Local/Temp/tps-restore`→`mv .git tps-backup/.git-corrupted-20260818`+`cp -r tps-restore/.git .`→`git fsck --full` 干净→`git add -A && git commit && git push`。commit `54c9bc2`（41 文件 +15622/-13329），推送 `e2c71d1..54c9bc2 main -> origin/main` 成功。**.gitignore 新增**：`.workbuddy/skills/`/`.workbuddy/redbox-screenshot.png`/`.workbuddy/sessions/`/`.workbuddy/mcp.json`/`.workbuddy/settings.json`（WorkBuddy 本地产物不入库）+ `20260812-MCP协议-深度分析报告.md`/`前端审查报告.html`（本地临时报告）。**后续 git 损坏修复铁律**：直接 reclone + 替换 .git，不要再试 `git gc`/`read-tree` 等修补，修补不彻底会复发。
